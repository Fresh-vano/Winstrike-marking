...
def validation(model, criterion, evaluation_loader, converter, opt, device):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        if 'CTC' in opt.Prediction:
            ...
            # Вычисление значения потерь для кодера CTC.
            preds_size = torch.IntTensor([preds.size(1)] * batch_size)
            # Перестановка осей в 'preds', чтобы использовать в CTCloss
            cost = criterion(preds.log_softmax(2).permute(1, 0, 2), text_for_loss, preds_size, length_for_loss)

            if opt.decode == 'greedy':
                # Выбор с максимальной вероятностью, затем декодирование индекса в символ
        ...
        # Вычисление точности и уверенности
