...
def contrast_grey(img):
    # Рассчитать контраст изображения, используя 90-й и 10-й процентиль
    high = np.percentile(img, 90)
    low = np.percentile(img, 10)
    return (high - low) / (high + low), high, low

def adjust_contrast_grey(img, target=0.4):
    # Регулировка контраста изображения до целевого уровня
...
class OCRDataset(Dataset):
    ...
    def __init__(self, root, opt):
        ...
        self.opt = opt
        print(root)
        # Загрузка данных из CSV файла с метками
    ...
    def __getitem__(self, index):
        ...
        if self.opt.rgb:
            img = Image.open(img_fpath).convert('RGB')  # для цветного изображения
        else:
...
class ResizeNormalize(object):
    ...
    def __call__(self, img):
        # Изменение размера изображения и его нормализация
...
class NormalizePAD(object):
    ...
    def __call__(self, img):
        # Нормализация и добавление отступов к изображению
        img = self.toTensor(img)
        img.sub_(0.5).div_(0.5)
        c, h, w = img.size()
        Pad_img = torch.FloatTensor(*self.max_size).fill_(0)
        Pad_img[:, :, :w] = img  # добавление отступов справа
        if self.max_size[2] != w:  # добавление границы
...
def tensor2im(image_tensor, imtype=np.uint8):
    # Преобразование тензора изображения в numpy массив
