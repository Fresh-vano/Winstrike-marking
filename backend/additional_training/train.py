...
def train(opt, show_number=2, amp=False):
    ...
    valid_loader = torch.utils.data.DataLoader(
        ...
        shuffle=True,  # 'True' для проверки прогресса обучения с помощью функции валидации.
    ...
    if opt.saved_model != '':
        ...
        for name, param in model.named_parameters():
            if 'localization_fc2' in name:
                print(f'Пропуск {name}, так как он уже инициализирован')
                continue
            try:
                if 'bias' in name:
                    init.constant_(param, 0.0)
    ...
    print('Количество обучаемых параметров: ', sum(params_num))
    ...
    if opt.saved_model != '':
        try:
            ...
            print(f'продолжение обучения, start_iter: {start_iter}')
        except:
            pass
    ...
    while True:
        ...
        if (i % opt.valInterval == 0) and (i != 0):
            print('время обучения: ', time.time() - t1)
            t1 = time.time()
            ...
            with open(f'./saved_models/{opt.experiment_name}/log_train.txt', 'a', encoding="utf8") as log:
                ...
                current_model_log = f'{"Текущая точность":17s}: {current_accuracy:0.3f}, {"Текущий норм. ED":17s}: {current_norm_ED:0.4f}'

                # сохранение модели с лучшей точностью (на валид. датасете)
                ...
                if current_norm_ED > best_norm_ED:
                    ...
                    best_model_log = f'{"Лучшая точность":17s}: {best_accuracy:0.3f}, {"Лучший норм. ED":17s}: {best_norm_ED:0.4f}'
                    loss_model_log = f'{loss_log}\n{current_model_log}\n{best_model_log}'
                    print(loss_model_log)
                    log.write(loss_model_log + '\n')
                    dashed_line = '-' * 80
                    head = f'{"Ожидаемое значение":25s} | {"Предсказание":25s} | Уверенность и Верно/Ложно'
                    ...
                    print('время валидации: ', time.time() - t1)
                    t1 = time.time()
                # сохранение модели каждые 1e+4 итераций.
                if (i + 1) % 1e+4 == 0:
